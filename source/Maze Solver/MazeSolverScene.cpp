...
namespace MazeSolver {
	void MazeSolverScene::will_become_current(ISceneManager * manager) {
...

		{
			...
			Mat44 transform = translate(Vec3(-10, 0, 10)) << rotate<Y>(R90);
	...
	void MazeSolverScene::reset_maze () {
		...
		*/
		...
		// Set the goal:
		_path_finder = new MazePathFinder(vector(0, 0), vector(39, 19), _maze.get());
		_maze->at(vector(39, 19))->floor_color = vector(1.0, 0.0, 0.0);
	}
			...
			for (std::size_t y = 0; y < size[Y]; y += 1) {
				for (std::size_t x = 0; x < size[X]; x += 1) {
...

					{
						...
						binding.set_uniform("model_matrix", Mat44(translate(center)));
						_floor_mesh_buffer->draw();
					}

					binding.set_uniform("major_color", Vec4(0.5, 0.3, 0.8, 1.0));

					Mat44 translate_to_center = translate(center);
					Mat44 north_wall_transform = translate_to_center << rotate<Z>(-R90);
					Mat44 south_wall_transform = translate_to_center << rotate<Z>(R90);
					Mat44 east_wall_transform = translate_to_center << rotate<Z>(R180);
					Mat44 west_wall_transform = translate_to_center;

					if (tile->walls & Maze::NORTH) {
						binding.set_uniform("model_matrix", north_wall_transform);
						_wall_mesh_buffer->draw();
					}

					if (tile->walls & Maze::SOUTH) {
						binding.set_uniform("model_matrix", south_wall_transform);
						_wall_mesh_buffer->draw();
					}

					if (tile->walls & Maze::EAST) {
						binding.set_uniform("model_matrix", east_wall_transform);
						_wall_mesh_buffer->draw();
					}

					if (tile->walls & Maze::WEST) {
						binding.set_uniform("model_matrix", west_wall_transform);
						_wall_mesh_buffer->draw();
					}
				}
			}
